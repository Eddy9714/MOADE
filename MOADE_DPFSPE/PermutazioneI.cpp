...
void PermutazioneI::prodotto(double F) {
	if (F >= 0) {
		...
		Coppia<unsigned int>* coppie;

		if (F < 1) {
			coppie = new Coppia<unsigned int>[(int)ceil(dimensione * F)];
			...
			coppie = new Coppia<unsigned int>[limite];
...
void PermutazioneI::randomIS(PermutazioneI* p, unsigned short& spostamenti, double F, Coppia<unsigned int>* risultato, int limiteSpostamenti){
...
void PermutazioneI::inserisci(PermutazioneI* p, Coppia<unsigned int>& c) {
...
unsigned int PermutazioneI::distanzaID() {
	return insWeight(this);
}

unsigned int PermutazioneI::distanzaRevID() {
	return this->dimensione - 1 - distanzaID();
}